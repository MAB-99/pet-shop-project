...
import toast from 'react-hot-toast'; // Usamos toast para notificaciones bonitas
import useAuth from '../../hooks/useAuth';
import ProductTable from './ProductTable';
import ProductForm from '../../components/ProductForm';
import { API_URL } from '../../lib/constants';

// --- PESTAÑA DE TURNOS (CORREGIDA) ---
const AppointmentsTab = () => {
    ...
    const fetchAppointments = async () => {
        try {
            ...
            // Nota: Asegúrate si tu ruta en el backend es 'appointment' o 'appointments'
    ...
    const handleStatusChange = (id, newStatus) => {
        if (newStatus === 'Confirmado') {
            setConfirmingId(id); // Abrir modal
        } else {
            saveStatus(id, newStatus); // Guardar directo
        }
    };
    ...
    const saveStatus = async (id, newStatus, date = null) => {
        try {
            ...
            // 🔴 ANTES: /api/appointment/${id}/status (Daba error 404)
            // 🟢 AHORA: /api/appointment/${id} (Correcto)
            ...
            if (response.ok) {
                toast.success(`Turno ${newStatus} correctamente`);
                fetchAppointments();
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative">
            <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                <Calendar className="text-purple-600" /> Gestión de Peluquería
            </h3>
            ...
            {selectedImage && (
                <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setSelectedImage(null)}>
                    ...
                    <img src={selectedImage} alt="Zoom" className="max-h-[90vh] max-w-full rounded-lg shadow-2xl" onClick={(e) => e.stopPropagation()} />
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6">
                        ...
                        <div className="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200 max-h-48 overflow-y-auto">
                            <p className="text-xs font-bold text-gray-500 uppercase mb-2">📅 Ocupados:</p>
                            {getBusySlots().length === 0 ? (
                                <p className="text-sm text-gray-400 italic">Agenda libre.</p>
                            ) : (
                        ...
                        <div className="flex gap-3 justify-end">
                            <button onClick={() => setConfirmingId(null)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">Cancelar</button>
                            <button
                                onClick={() => {
                    ...
                    <div key={appt._id} className={`border rounded-lg p-4 flex flex-col justify-between transition-all ${appt.status === 'Confirmado' ? 'bg-white border-green-200 shadow-md ring-1 ring-green-100' : 'bg-gray-50 border-gray-200'}`}>
                        <div>
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className={`px-2 py-1 text-xs rounded-full border font-bold ${appt.status === 'Confirmado' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-yellow-100 text-yellow-700 border-yellow-200'}`}>
                                    {appt.status}
                                </span>
                            </div>
                            <div className="w-full h-48 bg-gray-200 rounded-md mb-4 overflow-hidden cursor-zoom-in group relative" onClick={() => setSelectedImage(appt.petPhoto)}>
                            ...
                            {appt.status === 'Confirmado' && appt.confirmedDate && (
                                <div className="bg-green-50 p-2 rounded mb-3 border border-green-100">
                                    <p className="text-xs text-green-800 font-bold flex items-center gap-1">
                                        <CalendarIcon size={12} /> {new Date(appt.confirmedDate).toLocaleDateString()}
                                        <Clock size={12} className="ml-1" /> {new Date(appt.confirmedDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}hs
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="space-y-3 mt-2 pt-3 border-t border-gray-100">
                            <a href={`https://wa.me/${appt.contactPhone}?text=Hola, te contacto por el turno de ${appt.petName}.`} target="_blank" rel="noreferrer" className="block w-full text-center bg-green-500 text-white py-2 rounded hover:bg-green-600 transition font-medium flex items-center justify-center gap-2">
                                <span>💬 WhatsApp</span>
                            </a>
                            <select className="block w-full border border-gray-300 rounded p-2 text-sm bg-white" value={appt.status} onChange={(e) => handleStatusChange(appt._id, e.target.value)}>
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
                <LayoutDashboard className="text-blue-600" /> Resumen General
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
                    <p className="text-blue-600 text-sm font-semibold">Ventas Totales</p>
                    <p className="text-2xl font-bold text-blue-900">${stats.totalSales?.toLocaleString('es-AR')}</p>
                </div>
                ...
                <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-100">
                    <p className="text-yellow-600 text-sm font-semibold">Productos</p>
                    <p className="text-2xl font-bold text-yellow-900">{stats.totalProducts}</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                {view === 'table' && (
                    <button onClick={handleNew} className="bg-gray-900 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-800 shadow-md">
...
const OrdersTab = () => {
    ...
    const fetchOrders = async () => {
        try {
            const token = localStorage.getItem('token');
            // Usamos /api/order (singular) por consistencia
    ...
    // Marcar como entregado (Dispara notificación)
    const handleDeliver = async (orderId) => {
        if (window.confirm('¿Confirmar que el pedido fue entregado/enviado?')) {
            try {
                const token = localStorage.getItem('token');
                ...
                if (response.ok) {
                    toast.success('Orden marcada como enviada/entregada 🚚');
                    fetchOrders();
                } else {
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                <ShoppingBag className="text-blue-500" /> Gestión de Órdenes
            </h3>
            {orders.length === 0 ? (
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {orders.map((order) => (
                                <tr key={order._id} className="hover:bg-gray-50 transition-colors">
                                    <td className="p-4 text-xs font-mono text-gray-500">#{order._id.slice(-6)}</td>
                                    <td className="p-4 text-sm font-medium text-gray-800">
                                        {order.user?.name || "Eliminado"}
                                    ...
                                    <td className="p-4 flex items-center gap-2">
                                        ...
                                        {/* Indicador Entregado */}
                                        {order.isDelivered && (
                                            <span className="bg-green-100 text-green-600 p-2 rounded flex items-center gap-1 text-xs font-bold">
                                                <Check className="h-4 w-4" /> Entregado
                                            </span>
                                        )}

                                        {/* Select de estado manual (si se requiere) */}
                                        {!order.isDelivered && (
                                            <select
                        ...
                        <aside className="w-full md:w-64 flex-shrink-0">
                            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
                                <div className="p-6 border-b border-gray-50 bg-gray-900 text-white">
                                    ...
                                    <p className="text-xs text-gray-400">Hola, {auth.name}</p>
                                </div>
                                <nav className="p-2 space-y-1">
                                    {[
                                        ...
                                        { id: 'appointments', icon: Calendar, label: 'Peluquería' },
                                    ].map((item) => (
                                        <button
                                            key={item.id}
                                            onClick={() => setActiveTab(item.id)}
                                            className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-colors ${activeTab === item.id ? 'bg-yellow-50 text-yellow-700' : 'text-gray-600 hover:bg-gray-50'
                                                }`}
                                        >
